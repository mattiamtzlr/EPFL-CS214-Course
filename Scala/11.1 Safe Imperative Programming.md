...
## Lazy Fields
We can define analyze the above, by defining a class `LazyCell` which has a *lazy field*:
```Scala
...
> lc.get
   -> 42
```
where we observe the same behavior before with `lc.get` only being evaluated once.
